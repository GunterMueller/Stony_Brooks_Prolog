...
sold:    switch (TAG(op1)) {
      ...
      case mulreg:                /* PRR */
         ...
         switch (floatp) {
            ...
            case  0: temp_res = INTVAL(op2) * INTVAL(op1);
                     if ((POS_ANS(op1,op2) && POS_OVERFLOW(temp_res)) ||
			 (!POS_ANS(op1,op2) && NEG_OVERFLOW(temp_res)))
                        FOLLOW(op3) = makefloat((double)temp_res);
                     else
                        FOLLOW(op3) = MAKEINT(temp_res);
                     NEXT_INSTRUCTION;
            case  1: FOLLOW(op3) = makefloat(NUMVAL(op2) * NUMVAL(op1));
                     NEXT_INSTRUCTION;
         }
      ...
      case divreg:                /* PRR */
         ...
         DEREF(op1);
         if (ISNUM(op1))
            floatp = 0;           /* cvt'ing op1, op2 to double soon anyway */
         else {
            floatp = eval(op1, &top1);
            op1 = top1;
         }
         ...
         if (INTEGRAL(result)) {
            if ((POS_ANS(op1,op2) && POS_OVERFLOW(result)) ||
		(!POS_ANS(op1,op2) && NEG_OVERFLOW(result)))
               FOLLOW(op3) = makefloat(result);
            else FOLLOW(op3) = MAKEINT(result);
...
nunify4: switch (TAG(op2)) {
	       ...
	       if (op1 == op2)
                  NEXT_INSTRUCTION;
                        ...
                        prettymuch_equal((double)NUMVAL(op2),
                                         (double)NUMVAL(op1)))
                  NEXT_INSTRUCTION;
	       /* fails by default */
   ...
   switch (TAG(op1)) {
	 ...
	 if (ISINTEGER(op1) && INTVAL(op1) == op2)
            NEXT_INSTRUCTION;
                  ...
                  prettymuch_equal((double)NUMVAL(op1), (double)op2))
            NEXT_INSTRUCTION;
         /* fails by default */
      case CS  :
      case LIST:
...
nunify_with_float:                       /* op1 is unknown,                   */
   ...
   switch (TAG(op1)) {
      case FREE:
         ...
         NEXT_INSTRUCTION;
      case NUM:
         if (prettymuch_equal(NUMVAL(op1), NUMVAL(op2)))
            NEXT_INSTRUCTION;
         /* fails by default */
      case CS  :
      case LIST:
