 ...
 * if reg3 = 0 then reg1 is bound to a real and reg2 is a variable;
 * while if reg3 = 1 then reg1 is a variable and reg2 is bound to an integer.
 ...
 */
    ...
    op2 = reg[2];  DEREF(op2);
    op3 = reg[3];  DEREF(op3);
    if (INTVAL(op3) == 0) {    /* reg1 = float, reg2 = free */
       FOLLOW(op2) = MAKEINT((LONG)(floatval(op1)));
       PUSHTRAIL(op2);
    } else {                   /* reg1 = free, reg2 = integer */
