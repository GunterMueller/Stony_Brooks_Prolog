   ...
   while (!ISNIL(op2)) {
      ...
      if (ISATOM(op3)) {
         ...
         if (IS_ORDI(psc_ptr)) {
            ...
            call_arg[i].name = GET_NAME(psc_ptr);
      } else if (ISINTEGER(op3))
         call_arg[i].num = INTVAL(op3);
      else
...
b_BROCALL()
{  /* reg1: call #;
    * reg2: buffer containing args in 4 byte fields;
    * reg3: buffer to put return value in.
    */
   register LONG_PTR top;
   register LONG     op1, op;
   PSC_REC_PTR       psc_ptr;
   LONG_PTR          aptr;

   op1  = reg[1];  DEREF(op1);  /* brocall number */
   op   = reg[2];  DEREF(op);
   aptr = (LONG_PTR)GET_NAME(GET_STR_PSC(op));    /* buff with args */
   op   = reg[3];  DEREF(op);
   psc_ptr = GET_STR_PSC(op);                     /* buff for result */
   switch (INTVAL(op1)) {
      case  2: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG) getenv(*aptr);       break;

      /* Communication subsystem system calls.  Have not included byteorder
       * (ntohl, ntohs, htonl, htons).  Each call is from manual entry 3N,
       * except getpeername, which is from 2.
       */

/*    case 21: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)gethostent();
	       break;                                                      */
      case 22: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)gethostbyname(*aptr);
	       break;
      case 23: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)gethostbyaddr(*aptr);
	       break;
      case 24: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)sethostent(*aptr);
	       break;
      case 25: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)endhostent();
	       break;

      case 26: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getnetent();
	       break;
      case 27: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getnetbyname(*aptr);
	       break;
      case 28: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getnetbyaddr(*aptr);
	       break;
      case 29: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)setnetent(*aptr);
	       break;
      case 30: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)endnetent();
	       break;

      case 31: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getprotoent();
	       break;
      case 32: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getprotobyname(*aptr);
	       break;
      case 33: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getprotobynumber(*aptr);
	       break;
      case 34: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)setprotoent(*aptr);
               break;
      case 35: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)endprotoent();
	       break;

      case 36: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getservent();
	       break;
      case 37: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getservbyname(*aptr);
	       break;
      case 38: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)getservbyport(*aptr);
	       break;
      case 39: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)setservent(*aptr);
	       break;
      case 40: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)endservent();
	       break;

/*    case 41: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_addr(*aptr);
	       break;                                                      */
      case 42: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_network(*aptr);
	       break;
      case 43: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_ntoa(*aptr);
	       break;
/*    case 44: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_makeaddr(*aptr);
	       break;                                                      */
      case 45: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_lnaof(*aptr);
	       break;

      case 46: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)inet_netof(*aptr);
	       break;
/*    case 47: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG) get_peername(*aptr);
	       break;                                                      */
      case 50: *(LONG_PTR)GET_NAME(psc_ptr) = (LONG)perror(*aptr);
	       break;
